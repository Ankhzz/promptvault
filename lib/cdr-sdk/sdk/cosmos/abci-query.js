...
import { decodeCDRPartialsResponse, decodeLatestActiveResponse, decodeParamsResponse, decodeVerifiedRegistrationsResponse, encodeCDRPartialsRequest, encodeLatestActiveRequest, encodeParamsRequest, encodeVerifiedRegistrationsRequest, } from "./dkg-proto.js";
/**
 ...
 */
export async function abciQuery(rpcUrl, path, data) {
...
export async function queryLatestActiveDKGNetwork(rpcUrl) {
    const bytes = await abciQuery(rpcUrl, `${SERVICE}/GetLatestActiveDKGNetwork`, encodeLatestActiveRequest());
 ...
 */
export async function queryDKGParams(rpcUrl) {
    const bytes = await abciQuery(rpcUrl, `${SERVICE}/Params`, encodeParamsRequest());
    return decodeParamsResponse(bytes);
}
export async function queryVerifiedRegistrations(rpcUrl, round, codeCommitmentHex = "") {
    const bytes = await abciQuery(rpcUrl, `${SERVICE}/GetAllVerifiedDKGRegistrations`, encodeVerifiedRegistrationsRequest(round, codeCommitmentHex));
 ...
 */
export async function queryCDRPartials(rpcUrl, uuid, requesterPubKeyHex) {
    const hex = requesterPubKeyHex.replace(/^0x/i, "");
    try {
        const bytes = await abciQuery(rpcUrl, `${SERVICE}/GetCDRPartials`, encodeCDRPartialsRequest(uuid, hex));
...
//# sourceMappingURL=abci-query.js.map